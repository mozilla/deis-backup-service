...
cat s3cfg | ACCESS_KEY=${AWS_ACCESS_KEY} SECRET_KEY=${AWS_SECRET_KEY} HOST_BASE=s3.amazonawscom HOST_BUCKET=%\(bucket\)s.s3.amazonaws.com ENCRYPT=${ENCRYPT} PASSPHRASE=${PASSPHRASE} HTTPS=${AWS_HTTPS:-True} envsubst > ${AWS_CONFIG_FILE}

cat s3cfg | ACCESS_KEY=${CEPH_ACCESS_KEY} SECRET_KEY=${CEPH_SECRET_KEY} HOST_BASE=deis-store.${DEIS_DOMAIN} HOST_BUCKET=deis-store.${DEIS_DOMAIN} ENCRYPT=False PASSPHRASE=${PASSPHRASE} HTTPS=False envsubst > ${DEIS_CONFIG_FILE}

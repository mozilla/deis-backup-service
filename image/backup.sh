...
cat s3cfg | ACCESS_KEY=${AWS_ACCESS_KEY} SECRET_KEY=${AWS_SECRET_KEY} HOST_BASE=s3.amazonawscom HOST_BUCKET=%\(bucket\)s.s3.amazonaws.com ENCRYPT=${ENCRYPT} PASSPHRASE=${PASSPHRASE} HTTPS=${AWS_HTTPS:-True} REDUCED_REDUNDANCY=${AWS_REDUCED_REDUNDANCY:-False} envsubst > ${AWS_CONFIG_FILE}
